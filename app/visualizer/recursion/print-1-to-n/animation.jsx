...
function generatePrint1ToNFrames(n) {
  const frames = [];
  const stack = [];
  const printed = [];
...
const codeLines = [
  { line: 1, code: "function print1ToN(i, n) {" },
  { line: 2, code: "  if (i > n) return;" },
  { line: 3, code: "  console.log(i);" },
  { line: 4, code: "  print1ToN(i + 1, n);" },
  { line: 5, code: "}" },
];

const Print1ToNAnimation = () => {
  ...
  const frames = useMemo(() => {
    ...
    return generatePrint1ToNFrames(n);
  }, [nVal, isVisualizing]);
...
export default Print1ToNAnimation;
